...
module.exports = {
    type: 'group',
    command: ['werewolf', 'wwgc'],
    operate: async (context) => {
        const { ptz, m, q, vv, prefix, command, reply } = context;
const {
    emoji_role,
    sesi,
    playerOnGame,
    ...
    if (value === "create") {
        ...
        if (ww[chat].cooldown > 0) {
            if (ww[chat].time === "voting") {
                ...
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_pagi(ptz, chat, ww);
            }
        }
        if (ww[chat].status === true)
