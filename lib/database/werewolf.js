...
async function pagi(ptz, x, data) {
    ...
    return await ptz.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(ptz, x, data) {
    let row = [];
    let ment = [];
...
async function malam(ptz, x, data) {
...
async function skill(ptz, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await ptz.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, ptz, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await ptz.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(ptz, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(ptz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, ptz, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(ptz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, ptz, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(ptz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, ptz, data);
            break;
        } else {
            await skill(ptz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, ptz, data);
}

async function run_vote(ptz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, ptz, data);
            break;
...
async function run_malam(ptz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, ptz, data);
            break;
...
async function run_pagi(ptz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, ptz, data);
            break;
